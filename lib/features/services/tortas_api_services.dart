...
class ProductoApiService {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<dynamic> productosCategoria3 = jsonData
            .where((producto) => producto['idCategoriaProducto'] == 6)
            .toList();

        print('Productos de categoría 6 (Tortas): ${productosCategoria3.length}');
        print('=== FIN DEBUGGING ===');
 ...
 Future<String?> obtenerUrlImagen(int idImagen) async {
  ...
  Future<List<ProductModel>> obtenerProductosConImagenesOptimizado() async {
    ...
    .where((producto) => int.tryParse(producto['idCategoriaProducto'].toString()) == 6)
    .toList();

        
        print('Productos de categoría 3 (Tortas): ${productosCategoria3.length}');
