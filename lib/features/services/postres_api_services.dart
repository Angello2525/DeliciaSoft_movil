...
class ProductoApiService {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<dynamic> productosCategoria3 = jsonData
            .where((producto) => producto['idCategoriaProducto'] == 7 )
            .toList();

        print('Productos de categoría 3 (postres): ${productosCategoria3.length}');
        print('=== FIN DEBUGGING ===');
 ...
 Future<String?> obtenerUrlImagen(int idImagen) async {
  ...
  Future<List<ProductModel>> obtenerProductosConImagenesOptimizado() async {
    ...
    .where((producto) => int.tryParse(producto['idCategoriaProducto'].toString()) == 7)
    .toList();

        
        print('Productos de categoría 6 (postres): ${productosCategoria3.length}');
